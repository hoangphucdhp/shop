				...
				<div class="container px-6 mx-auto grid">
					...
					<form:form action="/category/index" modelAttribute="category"
						...
						<div
							...
							<form:label path="id" class="block text-sm">
							...
							<!-- HTML !-->
							<button onclick="confirmCreate(event)"
								formaction="/category/create" formmethod="post"
								class="buttonAdd">Create</button>
							<button onclick="confirmUpdate(event)"
								formaction="/category/update" formmethod="post"
								class="buttonUpdate">Update</button>
							<%--                         <button formaction="/category/delete/${item.id}" formmethod="post" class="buttonDelete">Delete</button>
 --%>
							<a class="btn btn-info mr-3 mt-2 mb-2" href="/admin/categories"><i
									...
									<tbody
										...
										<tr class="text-gray-700 dark:text-gray-400">
											...
											<td class="text-right"><a class="btn btn-primary mr-2"
												href="/category/edit/${item.id }" role="button">Edit</a></td>
											<td><a class="btn btn-danger mr-2"
												href="/category/delete/${item.id}" role="button"
												onclick="confirmDelete(event)">Delete<i
...
<script>
    function confirmDelete(event) {
        ...
        swal({
            ...
            if (confirmed) {
                window.location.href = event.target.href; // Chuyển hướng đến liên kết xóa
            }
        });
    }
    ...
    function confirmCreate(event) {
        ...
        swal({
            title: "Xác nhận",
            text: "Bạn có chắc chắn muốn tạo mới?",
            icon: "warning",
            buttons: true,
            dangerMode: true
        }).then((confirmed) => {
            if (confirmed) {
                ...
                event.preventDefault(); // Ngăn chặn hành động mặc định của nút "Create"
            }
        });
    ...
    function confirmUpdate(event) {
        ...
        swal({
            ...
            if (confirmed) {
                ...
                window.location.href = event.target.formAction; // Chuyển hướng đến liên kết xóa
            }
        });
